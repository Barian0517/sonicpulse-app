...
export class MusicFreeProvider implements MusicProvider {
    ...
    private async callPlugin(method: string, args: any[] = []): Promise<any> {
        ...
        const res = await fetch('http://:30001/plugin/call', {
            method: 'POST',
    ...
    async getPlugins(): Promise<any[]> {
        const res = await fetch(`http://:30001/plugins`);
        if (!res.ok) throw new Error('Failed to fetch plugins');
        return await res.json();
    }

    async installNetworkPlugin(url: string): Promise<any> {
        const res = await fetch(`http://:30001/plugin/install/url`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async installLocalPlugin(filePath: string): Promise<any> {
        const res = await fetch(`http://:30001/plugin/install/local`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async uninstallPlugin(id: string): Promise<any> {
        const res = await fetch(`http://:30001/plugin/uninstall`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async saveVariables(id: string, variables: Record<string, string>): Promise<any> {
        const res = await fetch(`http://:30001/plugin/variables`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async getStreamUrl(trackId: string, trackInfo?: any): Promise<string> {
        ...
        try {
            ...
            if (res && res.url) {
                let proxyUrl = `http://:30001/plugin/proxy?url=${encodeURIComponent(res.url)}`;
